...
This is a **Laravel Zero** CLI application called "wholefoods-data". Laravel Zero is a micro-framework built on Laravel components, optimized for building command-line applications.
...
```bash
./wholefoods-data
./wholefoods-data inspire
```

### Run Tests
```bash
./vendor/bin/pest           # Run all tests
./vendor/bin/pest --filter InspireCommandTest  # Run specific test
```
...
```bash
php wholefoods-data app:build
```

## Architecture

- **Entry point**: `wholefoods-data` (executable PHP script)
- **Commands**: `app/Commands/` - All CLI commands extend `LaravelZero\Framework\Commands\Command`
- **Service providers**: `app/Providers/`
- **Config**: `config/app.php` (app name, version, providers), `config/commands.php` (command paths, default command)
- **Tests**: Pest PHP framework with Feature and Unit directories

## Creating New Commands

Commands live in `app/Commands/` and auto-register via the `paths` config in `config/commands.php`. Each command needs:
- `$signature` - Command name and arguments
- `$description` - Help text
- `handle()` method - Command logic

Use Termwind's `render()` function for styled terminal output.

## Web Scraping & HTTP Conventions

- Use Laravel's `Http` facade (not raw Guzzle) for HTTP requests
- Use `Http::fake()` in tests - never make real HTTP requests in tests
- Saloon for structured API integrations (reusable request classes)
- Spatie Crawler for full website crawling
- Place Saloon connectors in `app/Http/Integrations/`
- Place CrawlObservers in `app/Crawlers/`
